...
(() => {
  ...
  const handlers = {
    ...
    setElevation: (v) => { const t = state.tiltLimit; state.elevation = Math.max(-t, Math.min(t, v)); },
    setFovH: (v) => { state.fovH = v; },
    setFovV: (v) => { state.fovV = v; },
    setRangeKm: (v) => { state.rangeKm = Math.min(state.coverageRange, v / 1000); },
    setTiltLimit: (v) => { state.tiltLimit = v; state.elevation = Math.max(-v, Math.min(v, state.elevation)); },
  ...
  function compute() {
    const az = state.azimuth, elv = state.elevation, fovH = state.fovH, fovV = state.fovV,
          ...
          pan = state.panLimit, covAz = state.coverageAzimuth;
    ...
    const edge = tilt + fovV / 2;
    const cUp = pE(edge, REcov), cDn = pE(-edge, REcov);
    const luP = pE(tilt, REcov), ldP = pE(-tilt, REcov);
    ...
    const elevDynamic = [
      el('path', { key: 'cov', d: `M${px},${py} L${f(cUp[0])},${f(cUp[1])} A${REcov},${REcov} 0 ${(2 * edge > 180) ? 1 : 0} 1 ${f(cDn[0])},${f(cDn[1])} Z`, fill: slateFill, stroke: slate, strokeWidth: 1.2, strokeDasharray: '6 4' }),
      ...
      el('text', { key: 'lut', x: f(luP[0] + 6), y: f(luP[1] - 2), fontFamily: mono, fontSize: 10, fill: ink }, '+' + tilt + '° raised'),
      el('text', { key: 'ldt', x: f(ldP[0] + 6), y: f(ldP[1] + 10), fontFamily: mono, fontSize: 10, fill: ink }, '−' + tilt + '° lowered'),
      el('text', { key: 'vext', x: f(cUp[0] - 6), y: f(cUp[1] - 4), fontFamily: mono, fontSize: 11, fill: '#6B7280', textAnchor: 'end' }, 'v_extent ' + vExt + '°'),
      ...
      el('text', { key: 'hz0', x: f(hzEnd + 7), y: py + 4, fontFamily: mono, fontSize: 10, fill: '#5B6472' }, 'horizon 0°'),
      el('line', { key: 'cvt', x1: f(hzEnd), y1: py - 6, x2: f(hzEnd), y2: py + 6, stroke: slate, strokeWidth: 1.4 }),
      el('text', { key: 'cvr', x: f(hzEnd + 7), y: py - 6, fontFamily: mono, fontSize: 10, fill: '#6B7280' }, (cov * 1000).toLocaleString() + ' m'),
      el('text', { key: 'fvr', x: f(bTip[0] + 5), y: f(bTip[1] - 3), fontFamily: mono, fontSize: 10, fill: amber }, (rangeKm * 1000).toLocaleString() + ' m')
    ].join('');
    ...
    return {
      ...
      coverageAzimuth: covAz, panLimit: pan, tiltLimit: tilt, coverageRange: cov,
  ...
  async function loadReport() {
    ...
    state.elevation = Math.max(-state.tiltLimit, Math.min(state.tiltLimit, state.elevation));
